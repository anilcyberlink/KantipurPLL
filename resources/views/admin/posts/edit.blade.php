...
@endsection
@section('content')
  <form class="form-horizontal" role="form" action="{{ url('admin/' . Request::segment(2) . '/' . $data->id) }}" method="post"
            ...
            <div class="col-lg-9">
              <div class="bs-component">
                ...
                <select name="category" class="form-control">
                  ...
                  <option value="{{$row->id}}" {{ ($row->id == $data->post_category )?'selected':'' }}> {{$row->category}}
                  </option>
                  @endforeach
                  @endif
                </select>
            ...
            <div class="col-lg-9">
              <div class="bs-component">
                <select name="post_parent" class="form-control">
                  ...
                  <option value="{{$row->id}}" {{ ($row->id == $data->post_parent)?'selected':'' }}>{{$row->post_title}}
                  </option>
                  @if(has_child_post($row->id))
                  @foreach(has_child_post($row->id) as $child_row)
                  <option value="{{$child_row->id}}" {{ ($child_row->id == $data->post_parent)?'selected':'' }}> —>
                    {{$child_row->post_title}}</option>
                  @if(has_child_post($child_row->id))
                  @foreach(has_child_post($child_row->id) as $grand_child_row)
                  <option value="{{$grand_child_row->id}}" {{ ($grand_child_row->id == $data->post_parent)?'selected':''
                  ...
                  <option value="{{$grand_child_row_x->id}}" {{ ($grand_child_row_x->id ==
                    $data->post_parent)?'selected':'' }}> — — —> {{$grand_child_row_x->post_title}}</option>
                  @endforeach
                  @endif
                  @endforeach
                  @endif
            ...
            <div class="col-lg-10">
              <div class="bs-component">
                <textarea class="form-control my-editor" id="editor2" name="post_content"
                  rows="20"> {{$data->post_content}}</textarea>
              </div>
            </div>
          </div>
